...
import codecs, time, re, unicodedata, math, os, os.path
...
# same as word graph but takes more edge lists
def build_word_char_graph(filename, nodes, edgeslist, hskmin, hskmax):
    print "Build word graph " + filename
    outnodes = set()
    outedges = set()
    foundorphans = {}
    ...
    for node in nodes:
        ...
        for e in edgeslist:
            if node in e:
                edges = edges.union(e[node])
        for destnode in edges:
            if (destnode in nodes):
                ...
                foundorphans[destnode].add(node)
    for orphan in foundorphans:
        ...
        if len(orphanedges) + len(foundorphans[orphan]) >= 2 or orphan in outnodes:
            # not an orphan, output it
            if orphan not in outnodes:
                graph.add_node(orphan, False, query_hsk_level(orphan, hskmin, hskmax))
                outnodes.add(orphan)
            for destnode in orphanedges:
                if (orphan, destnode) not in outedges:
                    graph.add_edge(orphan + "to" + destnode, orphan, destnode)
                    outedges.add( (orphan, destnode) )
            for fromnode in foundorphans[orphan]:
                if (fromnode, orphan) not in outedges:
                    graph.add_edge(fromnode + "to" + orphan, fromnode, orphan)
...
# print u"妈", ",".join(cc_components[u"妈"]) if u"妈" in cc_components else "False"

build_word_char_graph(prev + "testwordchar" + ext, [u"爱好",u"女",u"好",u"妈妈"],  [hsk_components, cc_components], 1, 1)
...
if True:
    build_word_char_graph(prev + "HSK1_wordchar" + ext,   hsk1words,  [hsk_components, cc_components], 1, 1)
    build_word_char_graph(prev + "HSK1-2_wordchar" + ext, hsk12words, [hsk_components, cc_components], 1, 2)
    build_word_char_graph(prev + "HSK1-3_wordchar" + ext, hsk13words, [hsk_components, cc_components], 1, 3)
    build_word_char_graph(prev + "HSK1-4_wordchar" + ext, hsk14words, [hsk_components, cc_components], 1, 4)
    build_word_char_graph(prev + "HSK1-5_wordchar" + ext, hsk15words, [hsk_components, cc_components], 1, 5)
    build_word_char_graph(prev + "HSK1-6_wordchar" + ext, hsk16words, [hsk_components, cc_components], 1, 6)
