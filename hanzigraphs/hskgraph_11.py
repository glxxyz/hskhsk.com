...
class GraphvizGraphLR(Graph):
    ...
    def add_dnw_node(self, identifier, label, iscur, isradical, isword, islearned):
        if iscur and isradical:
            self.nodes.append(identifier + u' [color="red" shape="box" margin="0.15,0.15" label="{}"]\n'.format(label))
        elif iscur:
            self.nodes.append(identifier + u' [color="red" label="{}"]\n'.format(label))
        elif isword and islearned:
            self.nodes.append(identifier + u' [color="black" label="{}"]\n'.format(label))
        elif isword:
            self.nodes.append(identifier + u' [color="black" style="dashed" label="{}"]\n'.format(label))
        elif isradical and islearned:
            self.nodes.append(identifier + u' [color="grey" shape="square" label="{}"]\n'.format(label))
        elif isradical:
            self.nodes.append(identifier + u' [color="grey" shape="box" margin="0.15,0.15" style="dashed" label="{}"]\n'.format(label))
        elif islearned:
            self.nodes.append(identifier + u' [color="grey" label="{}"]\n'.format(label))
        else:
            self.nodes.append(identifier + u' [color="grey" style="dashed" label="{}"]\n'.format(label))
...
def add_dnw_graph_subnodes(graph, words, main, hanzi, current_subgraph, output_nodes_edges):
    for component in dnw_subcomponents_alllevels[hanzi]:
        add_dnw_graph_subnodes(graph, words, main, component, current_subgraph, output_nodes_edges)
        if (component + "to" + hanzi) not in output_nodes_edges:
            output_nodes_edges.add(component + "to" + hanzi)
            graph.add_dnw_edge("p" + main + "s" + component, "p" + main + "s" + hanzi, len(hanzi) > 1)
    if hanzi not in output_nodes_edges:
        output_nodes_edges.add(hanzi)
        graph.add_dnw_node("p" + main + "s" + hanzi, hanzi + "\\n" + query_pinyin(hanzi), hanzi in current_subgraph, hanzi in cc_radicals, hanzi in words, hanzi in dnw_learned)
            
def add_dnw_graph_supernodes(graph, words, main, hanzi, current_subgraph, output_nodes_edges):
    if hanzi not in output_nodes_edges:
        output_nodes_edges.add(hanzi)
        graph.add_dnw_node("p" + main + "s" + hanzi, hanzi + "\\n" + query_pinyin(hanzi), hanzi in current_subgraph, hanzi in cc_radicals, hanzi in words, hanzi in dnw_learned)
    for component in dnw_supercomponents_alllevels[hanzi]:
        if (hanzi + "to" + component) not in output_nodes_edges:
            output_nodes_edges.add(hanzi + "to" + component)
            graph.add_dnw_edge("p" + main + "s" + hanzi, "p" + main + "s" + component, len(component) > 1)
        add_dnw_graph_supernodes(graph, words, main, component, current_subgraph, output_nodes_edges)


def is_same_dnw_sub_graph_super(subgraph_nodes, hanzi):
    if hanzi in subgraph_nodes:
        return True
    if len(dnw_supercomponents_alllevels[hanzi]) > 0:
        for component in dnw_supercomponents_alllevels[hanzi]:
            if is_same_dnw_sub_graph_super(subgraph_nodes, component):
                return True
    elif is_same_dnw_sub_graph_sub(subgraph_nodes, hanzi):
        return True
    return False

def is_same_dnw_sub_graph_sub(subgraph_nodes, hanzi):
    if hanzi in subgraph_nodes:
        return True
    for component in dnw_subcomponents_alllevels[hanzi]:
        if is_same_dnw_sub_graph_sub(subgraph_nodes, component):
            return True
    return False;
...
def dnw_analysis(compfilename, wordsfilename, dotfilename, words):
    ...
    while len(dnw_weight) > 0:
        ...
        if len(subgraph_nodes[subgraph_num]) and not is_same_dnw_sub_graph_super(subgraph_nodes[subgraph_num], maxhanzi):
            # start a new subgraph
            subgraph_num += 1
            subgraph_nodes.append(set())
        subgraph_nodes[subgraph_num].add(maxhanzi)
    for i in range(subgraph_num + 1):
        subgraph = subgraph_nodes[i]
        output_nodes_edges = set()
        for hanzi in subgraph:
            dnw_learned.add(hanzi)
            add_dnw_graph_subnodes(graph, words, str(i), hanzi, subgraph, output_nodes_edges)
            add_dnw_graph_supernodes(graph, words, str(i), hanzi, subgraph, output_nodes_edges)
