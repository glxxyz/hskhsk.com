...
# March 2013 Alan Davies alan@hskhsk.com
...
def query_hsk_word_level(somebit, hskmin=1, hskmax=6):
    level = 0
    if somebit in hsk_word_level:
        level = hsk_word_level[somebit]
    if level < hskmin or level > hskmax:
        level = 0
    return level

def query_hsk_char_level(somebit, hskmin=1, hskmax=6):
    level = 0
    if somebit in hsk_char_level:
        level = hsk_char_level[somebit]
    if level < hskmin or level > hskmax:
        level = 0
    return level

def query_pinyin(somebit):
    if somebit in hsk_pinyin:
        return hsk_pinyin[somebit]
    if somebit in cedict_pinyin: # also check for pinyin in CCEDICT
        parsed = pinyin_numbers_to_tone_marks(cedict_pinyin[somebit].lower())
...
def query_trad(somebit):
    if somebit in hsk_trad_char:
        return hsk_trad_char[somebit]
    if somebit in cedict_trad: # also check for trad in CCEDICT
        return cedict_trad[somebit]
    print "Traditional character not found:", somebit
    if len(somebit) == 1:
        return somebit
    return "".join([query_trad(s) for s in somebit])
    ...
    for line in infile:
        ...
        if len(splitted) >= 4:
            ...
            if word != "":
                ...
                for somebit in word:
                    if somebit in hsk_char_level:
                        hsk_char_level[somebit] = min(hsk_char_level[somebit], hsklevel)
                    else:
                        hsk_char_level[somebit] = hsklevel
                if len(word) > 1:
...
class GephiGraph(Graph):
    ...
    def add_node_detail(self, identifier, isword, hsklevel, (r, g, b), size, freq, position=None):
...
class GraphvizGraph(Graph):
    ...
    def add_node_detail(self, identifier, isword, hsklevel, (r, g, b), size, freq, position=None):
        self.nodes.append(identifier + ' [color="#{:02x}{:02x}{:02x}"];\n'.format(r, g, b) )
...
def calculate_hsk_extra_things(hsklevel, filename):
    ...
    for (level, freq, word) in found:
        outfile.write(str(level) + "\t" + word + "\t" + str(int(math.e**freq + 0.5)) + "\n")
    outfile.close()
...
if False:
    ...
    calculate_hsk_freq_adjust("txt/HSK_Freq_adjust" + version_prefix + ".txt")
...
# buildgraphs(GephiGraph, "gexf/", version_prefix + ".gexf")
# buildgraphs(GraphvizGraph, "graphviz/", version_prefix + ".dot")
...
if False:
    try:
        ...
        pass
    calculate_optimum_hsk_order(1, "txt/HSK_1_optimised" + version_prefix + ".txt")
    calculate_optimum_hsk_order(2, "txt/HSK_2_optimised" + version_prefix + ".txt")
    calculate_optimum_hsk_order(3, "txt/HSK_3_optimised" + version_prefix + ".txt")
    calculate_optimum_hsk_order(4, "txt/HSK_4_optimised" + version_prefix + ".txt")
    calculate_optimum_hsk_order(5, "txt/HSK_5_optimised" + version_prefix + ".txt")
    calculate_optimum_hsk_order(6, "txt/HSK_6_optimised" + version_prefix + ".txt")
    calculate_opt_hsk_add_other(1, "txt/HSK_1_optim_add" + version_prefix + ".txt")
    calculate_opt_hsk_add_other(2, "txt/HSK_2_optim_add" + version_prefix + ".txt")
    calculate_opt_hsk_add_other(3, "txt/HSK_3_optim_add" + version_prefix + ".txt")
    calculate_opt_hsk_add_other(4, "txt/HSK_4_optim_add" + version_prefix + ".txt")
    calculate_opt_hsk_add_other(5, "txt/HSK_5_optim_add" + version_prefix + ".txt")
    calculate_opt_hsk_add_other(6, "txt/HSK_6_optim_add" + version_prefix + ".txt")
    calculate_hsk_extra_things(1, "txt/HSK_1_Extra" + version_prefix + ".txt")
    calculate_hsk_extra_things(2, "txt/HSK_2_Extra" + version_prefix + ".txt")
    calculate_hsk_extra_things(3, "txt/HSK_3_Extra" + version_prefix + ".txt")
    calculate_hsk_extra_things(4, "txt/HSK_4_Extra" + version_prefix + ".txt")
    calculate_hsk_extra_things(5, "txt/HSK_5_Extra" + version_prefix + ".txt")
    calculate_hsk_extra_things(6, "txt/HSK_6_Extra" + version_prefix + ".txt")
    calculate_hsk_extra_things(7, "txt/HSK_7_Extra" + version_prefix + ".txt")
